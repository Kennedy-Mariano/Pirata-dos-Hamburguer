"""
Conexão com o banco de dados SQLite e funções utilitárias.
Substitui a versão PostgreSQL para facilitar testes locais rápidos.
"""
import sqlite3
...
def get_connection():
    """Abre e retorna uma conexão SQLite com row factory em dict-like rows."""
...
def criar_pedido(conn, cliente_nome, cliente_telefone, itens):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO pedidos (cliente_nome, cliente_telefone) VALUES (?, ?);",
    ...
    for item in itens:
        cur.execute(
            "SELECT preco_venda, custo_unitario, quantidade_estoque FROM produtos WHERE id = ?;",
            (item["produto_id"],),
        )
        ...
        cur.execute(
            ...
            (pedido_id, item["produto_id"], item["quantidade"], preco_venda, custo_unitario),
        )
...
def registrar_alerta_reposicao(conn, produto_id, quantidade_atual, estoque_minimo):
    cur = conn.cursor()
    # cria tabela alertas_reposicao se não existir (segurança para dev)
    cur.execute(
        "CREATE TABLE IF NOT EXISTS alertas_reposicao (id INTEGER PRIMARY KEY AUTOINCREMENT, produto_id INTEGER, quantidade_atual INTEGER, estoque_minimo INTEGER, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);"
    )
    cur.execute(
        "INSERT INTO alertas_reposicao (produto_id, quantidade_atual, estoque_minimo) VALUES (?, ?, ?);",
...
def buscar_dados_powerbi(conn):
    """
    Gera os dados de vendas que antes vinham de uma view vw_powerbi_pedidos.
    Monta uma lista de dicionários com campos compatíveis com o restante do código.
    """
    cur = conn.cursor()
    cur.execute(
        """
        SELECT p.id as pedido_id, pr.nome as produto, ip.quantidade as quantidade,
               ip.preco_unitario as receita, ip.custo_unitario as custo_total,
               (ip.preco_unitario - ip.custo_unitario) * ip.quantidade as lucro, p.created_at as data_pedido
